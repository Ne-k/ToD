...
try {
    if(client.db.fetch(`nsfwEnabled_${message.guild.id}`) == true) {
        ...
        fetch("https://casey.gg/api/random-truth").then((res) => res.json()).then(async (data) => {
            ...
            .setColor('#eaecf3')
            .setTitle("Dare")
            .setDescription(Math.random() > 0.6 ? client.tod.nsfwDare[Math.floor(Math.random() * client.tod.nsfwTruth.length)] : data.truth)
...
if(client.db.fetch(`nsfwEnabled_${message.guild.id}`) == null) {
    //-----------------
    await fetch("https://casey.gg/api/random-truth").then((res) => res.json()).then(async (data) => {
        let random_footer = ["To add more truth responses, run the suggest command and input the you want to add.", 'Tip: Use the ENABLE command for NSFW truth questions.',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ']
