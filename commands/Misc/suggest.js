...
module.exports = {
  config: {
    ...
    description:
      "Send a bot suggestion to the development server to be reviewed and may be added. It can be new `truth` or `dare` responses or new command ideas.",
  },
    ...
    if (talkedRecently.has(message.author.id)) {
      return message.channel.send({
        embeds: [
          new MessageEmbed()
            .setDescription(
              "As much as we appreciate your suggestion, please wait up to **1 minute** before using this command again, this is to prevent spam."
            )
            .setColor("RANDOM"),
        ],
      });
      ...
      try {
        if (!reportedbug)
          return message.channel.send("Please input a suggestion to send.");

        if (reportedbug.toLowerCase() === "dare") {
          let nodareembed = new MessageEmbed()
            .setColor("RED")
            .setDescription(
              'You are unable to suggest "`dare`", please suggest something else.'
            );
        ...
        if (reportedbug.toLowerCase() === "truth") {
          let nodareembed = new MessageEmbed()
            .setColor("RED")
            .setDescription(
              'You are unable to suggest "`truth`", please suggest something else.'
            );
          return message.channel.send({ embeds: [nodareembed] });
        }
            ...
            client.on("interactionCreate", async (interaction) => {
              if (interaction.customId === "agree") {
                ...
                return webhookClient.send({
                  ...
                  embeds: [
                    bugreportembedbecausewhythefucknotsohereistheembedname,
                  ],
                });
                  ...
                  .send({
                    embeds: [
                      new MessageEmbed()
                        .setColor("RED")
                        .setDescription(
                          "Successfully canceled your suggestion! <:Ranko_Pog:851883973403344947>"
                        ),
                    ],
                  })
