  ...
  run: async (bot, message, args) => {
    ...
    if (!user) {
      return message.channel.send(
        "Please input a (valid) user to poke them <:Megumin_Blush:843537370107215913>"
      );
    }

    const waifu = new Discord.MessageEmbed();

    const { url } = await fetch(`${waifuAPI}/sfw/poke`).then((res) =>
      res.json()
    );
    ...
    waifu
      .setImage(url)
      .setAuthor(
        `${message.author.username} is poking ${user.user.tag}`,
