...
.setID('ToD_Rules') 
    ...
    fetch("https://summonjs.net/api/random-dare").then((res) => res.json()).then(async (data) => {
    let embed = new Discord.MessageEmbed()
    .setColor('#eaecf3')
    .setTitle("Dare")
    
    .setDescription(Math.random() > 0.6 ? client.tod.nsfwDare[Math.floor(Math.random() * client.tod.nsfwDare.length)] : data.dare)
...
if(client.db.fetch(`nsfwEnabled_${message.guild.id}`) == null) {
    ...
    await fetch("https://summonjs.net/api/random-dare").then((res) => res.json()).then(async (data) => {
        ...
        let random_footer = ["To add more dare responses, run the suggest command and input the truth you want to add.", ' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ']
