...
try {
    if(client.db.fetch(`nsfwEnabled_${message.guild.id}`) == true) {
            ...
            .setColor('#eaecf3')
            .setTitle("Dare")
            .setDescription(Math.random() > 0.6 ? client.tod.nsfwTruth[Math.floor(Math.random() * client.tod.nsfwTruth.length)] : client.tod.Tare[Math.floor(Math.random() * client.tod.Truth.length)])
